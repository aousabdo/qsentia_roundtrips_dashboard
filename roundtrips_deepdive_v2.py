...
# Simple export covering every ticker present in roundtrips
all_symbols = pd.Series(pd.unique(roundtrips[SYM]), name=SYM)
tier_assignments = pd.DataFrame({SYM: all_symbols})
tier_assignments["tier"] = tier_assignments[SYM].map(league["tier"])
tier_assignments.to_csv("symbol_tier_assignments.csv", index=False)
print("Saved: symbol_tier_assignments.csv")
